...
app.get("/api/funds", async (c) => {
  try {
    ...
    if (search) {
      conditions.push(
        sql`(${funds.name} LIKE ${"%" + search + "%"} OR ${funds.fundId} LIKE ${"%" + search + "%"})`
      );
    }
    ...
    const totalCount = await db.select({ count: sql<number>`count(*)` }).from(funds);
...
app.get("/api/funds/:id/nav", async (c) => {
  try {
    ...
    if (history.length === 0) {
      try {
        const apiHistory = await fetchHistoricalNav(fundId, period as "1M" | "3M" | "6M" | "1Y" | "3Y" | "5Y" | "ALL");
...
app.get("/api/nav/compare", async (c) => {
  try {
    ...
    // Group by fund
    const byFund = new Map<string, Array<{ date: string; nav: number }>>();
